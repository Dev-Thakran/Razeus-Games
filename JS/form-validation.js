...
function validateConfirmPassword() {
	...
	const confirmPasswordInvalid = document.getElementById(
...
function validatePhone() {
	...
	if (!phoneRegex.test(phone)) {
		phoneInvalid.textContent =
			"Please enter a valid NZ phone number (e.g. 0212345678 or +64212345678)";
		phoneInput.classList.add("is-invalid");
		phoneInput.classList.remove("is-valid");
		return false;
	} // Valid
	...
	if (dateOfBirthValue === "") {
		...
		if (birthDate > today) {
			...
			if (!ageValue || calculatedAge !== ageValue) {
				dateOfBirthInvalid.textContent =
					"Your date of birth does not match your age";
				dateOfBirth.classList.add("is-invalid");
				dateOfBirth.classList.remove("is-valid");
				return false;
			} else {
